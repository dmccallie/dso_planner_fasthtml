...
// tsc ./static/astronomy-utils.ts --outFile ./static/astronomy-utils.js 
//         --target ES2015 --module system --allowJs --checkJs false
//parse a traditional RA string into a real number of degrees 
//fixme - requires all three HMS values - allow for missing seconds?
export function parseRA(ra_string) {
...
export function parseDEC(dec_string) {
...
export function ra2real(hr, min, sec) {
...
export function degree2HMS(degrees) {
...
export function decimalToDMS(decimalDegrees) {
...
export function degree2DMS(degrees) {
...
export function mean_sidereal_time(now, lon) {
...
export function coord_to_horizon(utc, ra, dec, lat, lon) {
...
//   };
export function horiz_to_equitorial(obs_dt, alt, azi, obs_long, obs_lat) {
...
export function get_transit_time(utc, ra, dec, lat, lon) {
...
export function gpt_localSiderealTime(date, observerLongitude) {
    // const now = new Date();
    //const now = date;
...
function gpt_calculateTransitTime(date, lst, ra) {
    const ra_hours = ra / 15.0;
...
export function use_gpt_calculateTransitTime(date, ra, observerLongitude) {
    ...
    return new Date(date.getFullYear(), date.getMonth(), date.getDate(), Math.floor(transitTimeCivilAdjusted), Math.floor((transitTimeCivilAdjusted * 60) % 60), Math.floor((transitTimeCivilAdjusted * 3600) % 60));
}
// from GPT
...
export function altitude_of_dso(utc, ra, dec, lat_dms, lon_dms) {
...
export function generate_sample_dates_by_hour(start_date, hours_interval, intervals) {
    const sample_dates = [];
